...
function exportarCSV(notas: NotaPendente[], trimestre: string) {
  const header = "Aluno,Turma,Disciplina,MAC,PP,PT,Média,Estado\n"
  const rows = notas.map(n =>
    `"${n.aluno_nome}","${n.turma_nome || ''}","${n.disciplina_nome}",${n.trabalho ?? ''},${n.p1 ?? ''},${n.exame ?? ''},${n.media ?? ''},${n.media !== null && Number(n.media) >= 10 ? 'Aprovado' : 'Reprovado'}`
  ).join("\n")
...
<style>body{font-family:Arial,sans-serif;margin:24px;font-size:13px;}h1{color:#1e40af;}table{width:100%;border-collapse:collapse;}th{background:#f1f5f9;padding:10px 8px;text-align:left;border-bottom:2px solid #cbd5e1;}@media print{body{margin:12px;}}</style>
      ...
      <h1>Inst. Politécnico Maiombe IB Sequele Nº 3050</h1>
...
<table>
  <thead><tr>
    ...
    <th style="text-align:center">MAC</th><th style="text-align:center">PP</th>
    <th style="text-align:center">PT</th><th style="text-align:center">Média</th>
  </tr></thead>
  <tbody>${rows}</tbody>
</table>
      <p style="margin-top:32px;color:#9ca3af;font-size:11px;">Gerado pelo SGN — Inst. Politécnico do Maiombe IB Sequele Nº 3050 — ${new Date().toLocaleDateString('pt-AO')}</p>
</body></html>`
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b bg-muted/30">
                          ...
                          <th className="text-center py-3 px-2 font-semibold">MAC</th>
                          <th className="text-center py-3 px-2 font-semibold">PP</th>
                          <th className="text-center py-3 px-2 font-semibold">PT</th>
