...
export default function AlunoFrequenciasPage() {
  const router = useRouter()
  const { user, isAuthenticated } = useAuth()
  const { toast } = useToast()

  const alunoId = user?.type === "aluno" ? Number(user.id) : null
        ...
        <main className="p-6">
          <div className="max-w-3xl mx-auto space-y-6">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="text-2xl font-bold">Minhas Frequências</h1>
            ...
            {aluno && (
              <Card>
                <CardHeader>
                  ...
                  <p className="text-sm text-muted-foreground">{aluno.nome} · Nº {aluno.numero} · {aluno.turma_nome || "—"}</p>
                </CardHeader>
                <CardContent>
                  {loading ? (
